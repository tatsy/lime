#ifndef _NPR_VECTOR_FIELD_DETAIL_H_
#define _NPR_VECTOR_FIELD_DETAIL_H_
...
namespace lime {
	...
	namespace npr {
		...
		namespace {
			...
			void calcETF(cv::Mat& image, cv::Mat& etf, int ksize = 5, int maxiter = 3, EdgeDetector edgeDetector = EDGE_SOBEL) {
				...
				if (edgeDetector == EDGE_SOBEL) {
					...
					for (int y = 0; y < height; y++) {
						for (int x = 0; x < width; x++) {
							...
							gx.at<float>(y, x) = (float)sumx;
							gy.at<float>(y, x) = (float)sumy;
						}
					}
				}
				else {
			...
			void calcSST(cv::Mat& image, cv::Mat& sst, int ksize = 5, EdgeDetector edgeDetector = EDGE_SOBEL) {
				int width = image.cols;
				int height = image.rows;
				int dim = image.channels();
		...
		void calcVectorField(cv::InputArray input, cv::OutputArray angles, int ksize, VFieldType vfieldType, EdgeDetector edgeDetector)
		{
			...
			if (vfieldType == VECTOR_SST) {
				...
				for (int y = 0; y < height; y++) {
					for (int x = 0; x<width; x++) {
						...
						vfield.at<float>(y, x) = (float)atan2(-F, lambda1 - E);
					}
				}
			}
				...
				for (int y = 0; y < height; y++) {
					for (int x = 0; x < width; x++) {
						...
						vfield.at<float>(y, x) = (float)atan2(ty, tx);
					}
				}
			}
			else {
...
#endif /* _NPR_VECTOR_FIELD_DETAIL_H_ */
