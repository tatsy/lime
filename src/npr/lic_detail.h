...
namespace lime {
	...
	namespace npr {
		...
		namespace {
			...
			void lic_classic(const cv::Mat& img, cv::Mat& out, const cv::Mat& vfield, int L) {
					...
					ompfor(int y = 0; y < height; y++) {
						for (int x = 0; x < width; x++) {
							...
							for (int pm = -1; pm <= 1; pm += 2) {
								...
								while (l < L && cnt++ < 100) {
									int lx = (int)ceil(pt.x);
									int ly = (int)ceil(pt.y);
									...
									for (int c = 0; c < dim; c++) {
										out.at<float>(y, x*dim + c) += img.at<float>(ly, lx*dim + c) * (float)h;
									}
									weight += h;
							...
							for (int c = 0; c < dim; c++) {
								out.at<float>(y, x*dim + c) = (weight != 0.0) ? out.at<float>(y, x*dim + c) / (float)weight : img.at<float>(y, x*dim + c);
							}
						}
					}
			...
			void lic_eularian(const cv::Mat& img, cv::Mat& out, const cv::Mat& vfield, int L) {
				const int width = img.cols;
				const int height = img.rows;
				const int dim = img.channels();
				const int depth = CV_MAKETYPE(CV_32F, dim);
						...
						for (int x = 0; x < width; x++) {
							...
							for (int pm = -1; pm <= 1; pm += 2) {
								...
								while (++l < L) {
									...
									for (int c = 0; c < dim; c++) {
										out.at<float>(y, x*dim + c) += (float)(w * img.at<float>(py, px*dim + c));
									}
									weight += w;
			...
			void lic_runge_kutta(const cv::Mat& img, cv::Mat& out, const cv::Mat& vfield, int L) {
				const int width = img.cols;
				const int height = img.rows;
				const int dim = img.channels();
				const int depth = CV_MAKETYPE(CV_32F, dim);
		...
		void lic(cv::OutputArray out, cv::InputArray img, const cv::Mat& vfield, int L, LicAlgo algo_type) {
			...
			if (algo_type == LIC_CLASSIC) {
				lic_classic(tmp, outRef, vfield, L);
			}
			else if (algo_type == LIC_EULARIAN) {
				lic_eularian(tmp, outRef, vfield, L);
			}
			else if (algo_type == LIC_RUNGE_KUTTA) {
		...
		void vector2angle(cv::InputArray vfield, cv::OutputArray angle) {
			...
			for (int y = 0; y < height; y++) {
				for (int x = 0; x < width; x++) {
					...
					A.at<float>(y, x) = (float)atan2(vy, vx);
				}
			}
		}
		...
		void angle2vector(cv::InputArray angle, cv::OutputArray vfield, double scale) {
			...
			for (int y = 0; y < height; y++) {
				for (int x = 0; x < width; x++) {
					...
					T.at<float>(y, x * 2 + 0) = (float)(scale * cos(theta));
					T.at<float>(y, x * 2 + 1) = (float)(scale * sin(theta));
				}
			}
		}
